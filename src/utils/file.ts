...
const comparePaths =
  (type: 'lessThan' | 'greaterThan') => (a: string, b: string) => {
    const patternA = tryParsePattern(a);
    const patternB = tryParsePattern(b);
    const lengthA =
      typeof patternA === 'object' ? patternA.prefix.length : a.length;
    const lengthB =
      typeof patternB === 'object' ? patternB.prefix.length : b.length;
...
const options =
  tsconfigPath &&
  JSON5.parse(fs.readFileSync(tsconfigPath, 'utf-8')).compilerOptions;
...
export const completeEntriesFromPath = (fragment: string) => {
  try {
    if (options?.paths) {
      ...
      Object.entries(options.paths as Record<string, string[]>).forEach(
        ([key, patterns]) => {
          if (key === '.') return;
          ...
          if (patterns) {
            ...
            const isMatch =
              typeof pathPattern === 'object' &&
              isPatternMatch(pathPattern, fragment);
            const isLongestMatch =
              isMatch &&
              (matchedPath === undefined ||
                comparePaths('greaterThan')(key, matchedPath));
            ...
            if (isLongestMatch) {
              ...
              const pathPrefix = keyWithoutLeadingDotSlash.slice(
                0,
                keyWithoutLeadingDotSlash.length - 1,
              );
